...
public class TerrorGlobalNPC : GlobalNPC
{
    ...
    {
        return lateInstantiation && !entity.SpawnedFromStatue && entity.CanBeChasedBy() && !NPCLists.SafeNPCs.Contains(entity.type);
    }
    ...
    {
        if (target.GetModPlayer<LeadPlayer>().leadArmorSet)
        {
