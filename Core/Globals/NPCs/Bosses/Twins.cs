...
namespace TerrorMod.Core.Globals.NPCs.Bosses
{
    public class Twins : GlobalNPC
    {
        ...
        {
            WhiteCircle = ModContent.Request<Texture2D>(WhiteCirclePath);
        }
        ...
        public override void OnSpawn(NPC npc, IEntitySource source)
        {
            if (npc.type == NPCID.Retinazer)
            {
                ...
                {
                    NPC.NewNPC(npc.GetSource_FromAI(), (int)npc.Center.X, (int)npc.Center.Y, ModContent.NPCType<ForgottenSibling>());
                }
            }
        }
