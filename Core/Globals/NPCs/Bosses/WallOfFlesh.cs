...
public class WallOfFlesh : GlobalNPC
{
    ...
    public override void AI(NPC npc)
    {
        ...
        if (doFlamethrower)
        {
            if (AITimer % 5 == 0)
            {
                ...
                {
                    Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, Vector2.UnitX.RotatedBy(npc.rotation) * 20, ProjectileType<EyeFireButFire>(), npc.damage / 5, 1f);
                }
            }
        }
        else
        {
            if (AITimer % 120 == 0 && npc.life < npc.lifeMax * 0.5f)
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                {
                    ...
                    Projectile.NewProjectile(npc.GetSource_FromAI(), position, Vector2.Zero, ProjectileType<HungryCannon>(), npc.damage / 5, 1f, ai1: above * MathHelper.PiOver2);
                }
            }
        }
    ...
    public override void OnKill(NPC npc)
    {
        ...
        {
            player.AddBuff(BuffType<BeforeTheStormBuff>(), 72000);
        }
        if (Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            for (int i = 0; i < maxCores; i++)
            {
                ...
                while (counter < 1000)
                {
                    ...
                    if (!Main.tile[x, y].HasTile && Main.tile[x, y].WallType == 0
                        && !Main.npc.Any(n => n.active && n.type == NPCType<MechanicalCore>() && n.Center.Distance(new Vector2(x * 16, y * 16)) < minDistance))
                    {
                        NPC.NewNPC(npc.GetSource_FromAI(), x * 16, y * 16, NPCType<MechanicalCore>());
