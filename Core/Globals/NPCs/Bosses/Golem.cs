...
public class Golem : GlobalNPC
{
            ...
            if (AttackTimer > 300 && AttackTimer < 600)
            {
                if (AttackTimer % 60 == 0)
                {
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        ...
                        Projectile.NewProjectile(npc.GetSource_FromAI(), pos, Vector2.Zero, ProjectileType<DartTrap>(), npc.damage / 4, 1, ai1: -dir);
                    }
                }
            }
            if (AITimer % 600 == 0)
    ...
    public override void OnHitPlayer(NPC npc, Player target, Player.HurtInfo hurtInfo)
    {
        ...
        if (Main.rand.NextBool(4)) target.AddBuff(BuffType<Weight>(), 120);
    }
}
