...
public class LaserSkull : ModProjectile
{
    ...
    public override void SetStaticDefaults()
    {
        ...
        LeftSkull = Request<Texture2D>(LeftSkullPath);
        RightSkull = Request<Texture2D>(RightSkullPath);
    }
    ...
    public override void AI()
    {
        if (AITimer == 0)
        {
            ...
            if (WaitTime > Projectile.timeLeft)
            {
                Projectile.timeLeft = (int)WaitTime + 10;
            }    
        }
        ...
        if (AITimer >= WaitTime)
        {
            ...
            {
                var proj = Projectile.NewProjectileDirect(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.Zero, ProjectileType<DoomSphere>(), Projectile.damage, 1f, ai0: 0.75f, ai1: Rotation - MathHelper.PiOver2, ai2: 0);
                proj.timeLeft = 120;
